...
public class ModLoader
{
    ...
    public static void LoadMod()
    {
        ...
        harmony.PatchAll();
        MActionIds.PrepareToAid = ModManager.TryParse("PrepareToAid", out ActionId prepareAid) ? prepareAid : ModManager.RegisterEnumMember<ActionId>("PrepareToAid");
        MActionIds.AidReaction = ModManager.TryParse("AidReaction", out ActionId aidReaction) ? aidReaction : ModManager.RegisterEnumMember<ActionId>("AidReaction");
        ...
        if (AllFeats.GetFeatByFeatName(FeatName.TheInfiniteEye) is {} infiniteEye)
        {
            string rulesText = infiniteEye.RulesText.Replace("attack hints", "omnidirectional scan");
            infiniteEye.RulesText = rulesText;
        }
        ...
        {
            if (cr.Level >= 6 && cr.HasFeat(MFeatNames.SilentWhisper))
            {
        ...
        {
            AllFeats.GetFeatByFeatName(FeatName.PsiStrikes).Prerequisites.RemoveAll(prerequisite => prerequisite.Description.ContainsIgnoreCase("psychic"));
        };
    }
}