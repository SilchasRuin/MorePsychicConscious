...
using static PsychicExpanded.ConsciousMindFeat;
...
public abstract class ConsciousMind : ModData
{
    ...
    {
        SpellId psiCantrip1 = SpellId.Daze;
    ...
    internal static IEnumerable<Feat> ParallelFeats()
    {
        List<Spell> spells =
        [
            CreatePsychicSpellTemplate(SpellId.ProduceFlame),
            CreatePsychicSpellTemplate(SpellId.RayOfFrost),
            CreatePsychicSpellTemplate(MSpellIds.ThermalStasis),
            CreatePsychicSpellTemplate(SpellId.Daze),
            CreatePsychicSpellTemplate(MSpellIds.Message),
            CreatePsychicSpellTemplate(MSpellIds.ForbiddenThought),
            CreatePsychicSpellTemplate(SpellId.WarpStep),
            CreatePsychicSpellTemplate(SpellId.PhaseBolt),
            CreatePsychicSpellTemplate(MSpellIds.TesseractTunnel)
        ];
...
//use this to add Conservation of Energy
    ...
    private static QEffect OscillatingWaveSetup()
    {
        return new QEffect
        {
            ...
            YouBeginAction = (effect, action) =>
            {
                ...
                if (action.SpellId == SpellId.FireShield && effect.Owner.HasEffect(MQEffectIds.OscillatingWaveCold) && IsWaveSpell(action.SpellId))
                {
                    effect.Owner.AddQEffect(new QEffect
                    {
                        YouAcquireQEffect = (wave, effectToCheck) =>
    ...
    private static QEffect OscillatingWaveFire()
    {
        return new QEffect
        {
            ...
            AfterYouTakeAction = (effect, action) =>
            {
                if (action.SpellId == SpellId.FireShield && WaveSpells.Contains(action.SpellId))
                {
                    effect.ExpiresAt = ExpirationCondition.EphemeralAtEndOfImmediateAction;
                    effect.WhenExpires = qf => qf.Owner.AddQEffect(OscillatingWaveCold());
                }
    ...
    public static List<SpellId> WaveSpells { get; set; } =
    [
        SpellId.ConeOfCold, SpellId.Fireball, SpellId.ScorchingRay,
        SpellId.BurningHands, SpellId.RayOfFrost, SpellId.ProduceFlame
    ];
    

    private static bool IsWaveSpell(SpellId spell)
    {
        return WaveSpells.Contains(spell);
    }
    ...
    private static QEffect PrepareRecallAid(Creature aider)
    {
            ...
            ExpirationCondition.ExpiresAtStartOfYourTurn, aider, IllustrationName.NarratorBook)
        {
            ...
            StateCheck = qf =>
            {
                ...
                {
                    if (!qfTech.Owner.HasEffect(MQEffectIds.RecallTeachings))
    ...
    private static QEffect ReceiveRecallAid(QEffect preparation)
    {
        ...
        {
            Tag = preparation,
            Id = MQEffectIds.RecallTeachings,
