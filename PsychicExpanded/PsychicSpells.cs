...
public abstract class PsychicSpells : ModData
{
    public static void RegisterSpells()
    {
        ...
        MSpellIds.ForbiddenThought = ModManager.RegisterNewSpell("Forbidden Thought", 0,
            (_, _, spellLevel, inCombat, spellInformation) =>
            {
                ...
                CombatAction forbiddenThought = Spells.CreateModern(IllustrationName.Confusion, "Forbidden Thought",
                        [Trait.Cantrip, Trait.Enchantment, Trait.Mental, Trait.Level1PsychicCantrip, Trait.Psychic, Trait.Uncommon, Trait.IsHostile],
                    ...
                    .WithEffectOnChosenTargets(async (spell, self, targets) =>
                    {
                        ...
                        foreach (ICombatAction combatAction in targets.ChosenCreature!.Possibilities.CreateActions(false)
                                     .Where(action =>
        ...
        MSpellIds.ShatterMind = ModManager.RegisterNewSpell("Shatter Mind", 0,
            (_, owner, spellLevel, inCombat, spellInformation) =>
            {
                ...
                CombatAction shatterMind = Spells.CreateModern(IllustrationName.BrainDrain, "Shatter Mind",
                        [Trait.Cantrip, Trait.Evocation, Trait.Mental, Trait.Psychic, Trait.Level1PsychicCantrip, Trait.Uncommon],
                        ...
                        Target.Cone(amped ? owner != null && owner.HasEffect(MQEffectIds.ShatterMind60) ? 12 : 6 : 3).WithIncludeOnlyIf((at, cr) => cr.EnemyOf(at.OwnerAction.Owner)),
            ...
            ModManager.ReplaceExistingSpell(SpellId.Daze, 0, (owner, level, inCombat, spellInformation) =>
            {
                ...
                CombatAction daze = Spells.CreateModern(IllustrationName.Daze, "Daze",
                        ...
                        + (psychicAmpInformation != null && !inCombat
                            ? $"\n\n{{Blue}}{{b}}Amp{{/b}} The spell's damage changes to {S.HeightenedVariable(heighten, 1)}. If the target fails its Will save, until the end of its next turn, it gains weakness {S.HeightenedVariable(1+(level - 1)/2, 1)} to mental damage and takes a -1 status penalty to Will saves. On a critical failure, the weakness is {S.HeightenedVariable(3+(level - 1)/2, 3)} (in addition to the target being stunned 1). The weakness applies before daze deals damage.{{/Blue}}"
                            : amped ? $" {{Blue}}If the target fails its Will save, until the end of its next turn, it gains weakness {S.HeightenedVariable(1+(level - 1)/2, 1)} to mental damage and takes a -1 status penalty to Will saves. On a critical failure, the weakness is {S.HeightenedVariable(3+(level - 1)/2, 3)} (in addition to the target being stunned 1). The weakness applies before daze deals damage.{{/Blue}}" : ""),
                    ...
                    .WithGoodness((target, _, _) =>
                        target.OwnerAction.SpellcastingSource?.SpellcastingAbilityModifier + 1 ?? 0)
                    .WithEffectOnEachTarget(async (power, self, target, result) =>
                    {
                        ...
                        if (amped) diceFormula1 = DiceFormula.FromText($"{heighten}d10", "Daze");
                        if (power.SpellLevel >= 3 && !amped)
                        {
                        ...
                        if (result <= CheckResult.Failure && amped)
                        {
                            int amount = result == CheckResult.CriticalFailure ? 3 : 1;
                            target.AddQEffect(QEffect.DamageWeakness(DamageKind.Mental,
                                    amount+(level - 1)/2)
                                .WithExpirationAtEndOfOwnerTurn());
                            target.AddQEffect(new QEffect("Amped Daze", $"You gain weakness {S.HeightenedVariable(amount+(level - 1)/2, amount)} to mental damage and take a -1 status penalty to Will saves.", ExpirationCondition.ExpiresAtEndOfYourTurn, self, power.Illustration)
                            {
                                BonusToDefenses = (_, _, defense) => defense.IsSavingThrow() && defense == Defense.Will ? new Bonus(-1, BonusType.Status, "Daze", false) : null
                            });
                        }
                ...
                if (amped) daze.Description += $" {{Blue}}If the target fails its Will save, until the end of its next turn, it gains weakness {S.HeightenedVariable(1+(level - 1)/2, 1)} to mental damage and takes a -1 status penalty to Will saves. On a critical failure, the weakness is {S.HeightenedVariable(3+(level - 1)/2, 3)}  (in addition to the target being stunned 1). The weakness applies before daze deals damage.{{/Blue}}";
        ...
        {
            ModManager.RegisterActionOnEachSpell(daze =>
            {
                if (daze.SpellId != SpellId.Daze)
                    return;
                daze.Traits.Add(Trait.Level1PsychicCantrip);
                SpellInformation? spellInformation = daze.SpellInformation;
                if (spellInformation == null)
                    return;
                PsychicAmpInformation? psychicAmpInformation = spellInformation.PsychicAmpInformation;
                if (psychicAmpInformation != null)
                    daze.PsychicAmpInformation =  psychicAmpInformation;
                bool amped = psychicAmpInformation is { Amped: true };
                if (psychicAmpInformation == null) return;
                // ReSharper disable once ConditionIsAlwaysTrueOrFalseAccordingToNullableAPIContract
                bool inCombat = daze.Owner != null && daze.Owner.Battle != TBattle.Pseudobattle;
                int level = daze.SpellLevel;
                daze.Description = string.Empty;
                int heighten = level % 2 == 0 ? level - 1 : level;
                daze.WithDescription("You cloud the target's mind and daze it with a mental jolt.",
                    $"{{b}}Range{{/b}} {{Blue}}120 feet{{/Blue}}" +
                    $"\n\nDeal {(amped ? S.HeightenedVariable(1 + 2 * ((level - 1) / 2), 0) + "d10" : S.HeightenedVariable((level + 1) / 2, 0) + "d6")} mental damage.\n\nIf the target critically fails the save, it is also stunned 1."
                    + (!inCombat
                        ? $"\n\n{{Blue}}{{b}}Amp{{/b}} The spell's damage changes to {S.HeightenedVariable(heighten, 1)}d10. If the target fails its Will save, until the end of its next turn, it gains weakness {S.HeightenedVariable(1 + (level - 1) / 2, 1)} to mental damage and takes a -1 status penalty to Will saves. On a critical failure, the weakness is {S.HeightenedVariable(3 + (level - 1) / 2, 3)} (in addition to the target being stunned 1). The weakness applies before daze deals damage.{{/Blue}}" :
                        amped ? $" {{Blue}}If the target fails its Will save, until the end of its next turn, it gains weakness {S.HeightenedVariable(1+(level - 1)/2, 1)} to mental damage and takes a -1 status penalty to Will saves. On a critical failure, the weakness is {S.HeightenedVariable(3+(level - 1)/2, 3)} (in addition to the target being stunned 1). The weakness applies before daze deals damage.{{/Blue}}" : "")+
                    S.HeightenText(level, 1, inCombat, "{b}Heightened (+2){/b} The damage increases by 1d6."));
                daze.EffectOnOneTarget = null;
                daze.WithEffectOnEachTarget(async (power, self, target, result) =>
                {
                    DiceFormula diceFormula1 =
                        DiceFormula.FromText($"{1+(heighten-1)/2}d6", "Daze");
                    if (amped) diceFormula1 = DiceFormula.FromText($"{heighten}d10", "Daze");
                    if (result <= CheckResult.Failure && amped)
                    {
                        int amount = result == CheckResult.CriticalFailure ? 3 : 1;
        ...
        ModManager.ReplaceExistingSpell(SpellId.ProduceFlame, 0, (owner, level, inCombat, spellInformation) =>
        {
            ...
            CombatAction produceFlame = Spells.CreateModern(IllustrationName.ProduceFlame, "Produce Flame",
                ...
                .WithEffectOnEachTarget(async (spell, caster, target, result) =>
                {
                    ...
                    num2 = spell.SpellcastingSource?.SpellcastingAbilityModifier ?? caster.Spellcasting?.Sources.FirstOrDefault()?.SpellcastingAbilityModifier ?? 0;
        ...
        MSpellIds.ThermalStasis =
            ModManager.RegisterNewSpell("ThermalStasis", 0, (_, _, spellLevel, inCombat, spellInformation) =>
            {
                ...
                CombatAction thermalStasis = Spells.CreateModern(IllustrationName.ResistEnergy, "Thermal Stasis",
                        [Trait.Cantrip, Trait.Abjuration, Trait.Psychic, Trait.Level1PsychicCantrip, Trait.VerbalOnly, Trait.Uncommon],
        ...
        MSpellIds.EntropicWheel = ModManager.RegisterNewSpell("EntropicWheel", 0,
            (_, _, spellLevel, inCombat, spellInformation) =>
            {
                ...
                CombatAction entropicWheel = Spells.CreateModern(MIllustrations.FireColdWheel, "Entropic Wheel",
                        [Trait.Cantrip, Trait.Cold, Trait.Evocation, Trait.Fire, Trait.Psychic, Trait.VerbalOnly, Trait.DoNotShowInCombatLog, Trait.Uncommon],
        ...
        ModManager.ReplaceExistingSpell(SpellId.WarpStep, 0, (owner, level, inCombat, spellInformation) =>
        {
            ...
            if (owner != null)
            {
                ...
                {
                    if (qf.BonusToAllSpeeds?.Invoke(qf) is { BonusType: BonusType.Status } bonusStatus)
                        bonuses.Add(bonusStatus.Amount);
                }
            ...
            CombatAction warpStepSpell = Spells.CreateModern(IllustrationName.WarpStep, "Warp Step",
                ...
                $"You gain a +{(psyche ? "{Blue}10{/Blue}" : "5")}-foot status bonus to your Speed until the end of your turn. {(amped && level >= 4 ? "{Blue}You then teleport to a location within range or Stride twice{/Blue}." : "You then Stride twice.")}"
            ...
            warpStepSpell.WithEffectOnSelf(async (action, self) =>
            {
                QEffect warpStep = new("Warp Step",
                    $"You have a +{(psyche ? "10" : "5")} status bonus to Speed.",
                    ExpirationCondition.ExpiresAtEndOfSourcesTurn, self,
                ...
                self.AddQEffect(warpStep);
                if (amped && level >= 4 && await self.AskForConfirmation(IllustrationName.WarpStep, "Would you like to teleport instead of striding?", "Yes.", "No."))
                {
                    ...
                    CombatAction teleport = CombatAction.CreateSimple(self, "Warp", Trait.DoNotShowOverheadOfActionName,
                        Trait.DoNotShowInCombatLog).WithActionCost(0);
                    teleport.Target = Target.TileYouCanSeeAndTeleportTo(speed);
                    teleport.Illustration = IllustrationName.WarpStep;
                    await self.Battle.GameLoop.FullCast(teleport);
                    if (teleport.ChosenTargets.ChosenTile == null)
                    {
                        self.RemoveAllQEffects(qf => qf == warpStep);
                        action.RevertRequested = true;
                        return;
                    }
                    action.Traits.Add(Trait.Teleportation);
                    await CommonSpellEffects.Teleport(self, teleport.ChosenTargets.ChosenTile);
                }
                else
                {
                    if (!await self.StrideAsync("Choose where to Stride with Warp Step. (1/2)",
                            allowCancel: true))
                    {
                        self.RemoveAllQEffects(qf => qf == warpStep);
                        action.RevertRequested = true;
                    ...
                    {
                        int num = await self.StrideAsync("Choose where to Stride with Warp Step. (2/2)",
                            allowPass: true)
                            ? 1
                            : 0;
                    }
            ...
            warpStepSpell.Description += inCombat ? "" :
                "\n{Blue}{b}Amp Heightened (4th){/b} Instead of striding, you can teleport to a space within your line of sight and line of effect with a range equal to your double your Speed (after applying the status bonus from warp step). This grants the spell the teleportation trait.{/Blue}";
        ...
        MSpellIds.GhostlyShift = ModManager.RegisterNewSpell("GhostlyShift", 0,
            (_, caster, spellLevel, inCombat, spellInformation) =>
            {
                PsychicAmpInformation? psychicAmpInformation = spellInformation.PsychicAmpInformation;
                bool amped = (psychicAmpInformation != null ? (psychicAmpInformation.Amped ? 1 : 0) : 0) != 0;
                CombatAction ghostlyShift = Spells.CreateModern(IllustrationName.Blink, "Ghostly Shift", [Trait.Cantrip, Trait.Conjuration, Trait.Psychic, Trait.VerbalOnly, Trait.Level1PsychicCantrip, Trait.SpellWithDuration, Trait.Uncommon],
                            ...
                            AfterYouTakeAction = async (_, action) =>
                            {
                                if (amped && action.ActionId == MActionIds.GhostThrough && !self.HasEffect(MQEffectIds.Ghost))
                                {
                                    if (action.ChosenTargets.ChosenCreature is {} enemy)
                                    {
                                        ...
                                        CheckResult result = await CommonSpellEffects.RollSpellSavingThrowAsync(enemy, spell, Defense.Fortitude);
        ...
        MSpellIds.TesseractTunnel = ModManager.RegisterNewSpell("TesseractTunnel", 0,
            (_, caster, spellLevel, inCombat, spellInformation) =>
            {
                ...
                CombatAction tesseractTunnel = Spells.CreateModern(IllustrationName.DimensionDoor, "Tesseract Tunnel", [Trait.Cantrip, Trait.Conjuration, Trait.Psychic, Trait.Teleportation, Trait.NoHeightening, Trait.SpellWithDuration, Trait.Uncommon],
                ...
                if (amped)
                {
                    tesseractTunnel.WithEffectOnChosenTargets(async (spell, self, targets) =>
                    {
                        ...
                        Tile entrance = self.Space.CenterTile;
                        if (exit != null)
                        {
                ...
                else
                {
                    tesseractTunnel.WithEffectOnEachTarget(async (spell, self, _, _) =>
                    {
                        ...
                        if (await self.StrideAsync("Choose where to stride to to open the exit portal",
                                allowCancel: true))
                        {
                            Tile exit = self.Space.CenterTile;
                            TileQEffect entranceQf = Teleporter(exit, amped, self);
        ...
        bool amped)
    {
        ...
        {
            YouBeginAction = async (effect, combatAction) =>
            {
                Creature self = effect.Owner;
                if (combatAction.Name == actionName)
                {
                    CheckResult savingThrow = await CommonSpellEffects.RollSpellSavingThrowAsync(self, spell, Defense.Will);
                    await CommonSpellEffects.DealBasicDamage(spell, caster, self, savingThrow, 1 + spellLevel + "d6",
                        DamageKind.Mental);
...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            {
                Tile original = creature.Space.CenterTile;
                if (creature.EnemyOf(owner) || (creature.AnimationData.LongMovement?.Path != null && !Equals(creature.AnimationData.LongMovement?.Path.LastOrDefault(), creature.Space.CenterTile))) return;
                    ...
                    creature.AddQEffect(new QEffect(ExpirationCondition.Ephemeral)
                    {
                        ProvideContextualAction = qf =>
                        {
                            CombatAction enterTunnel = new CombatAction(qf.Owner, IllustrationName.DimensionDoor,
                                ...
                                {
                                    Tile original = self.Space.CenterTile;
                                    await CommonSpellEffects.Teleport(self, exit);
                            ...
                            effect.Owner.AddQEffect(new QEffect()
                            {
                                ...
                                AfterYouTakeAction = (qff, cAction) =>
                                {
                                    if (cAction.ActionId is ActionId.Stride
                                        ...
                                        !Equals(qff.Owner.Space.CenterTile, exit) && !Equals(qff.Owner.Space.CenterTile, entrance))
                                        qff.Value = 0;
                                    return Task.CompletedTask;
                                },
          ...
          }).WithEffectOnChosenTargets(async (spell, caster, targets) =>
          {
              foreach (Tile tile in targets.ChosenTiles)
              {
                  foreach (TileQEffect tileQEffect in (IEnumerable<TileQEffect>) tile.TileQEffects)
                  {
                      ...
                      {
                          Creature noncombatCreature = Creature.CreateNoncombatCreature(IllustrationName.DisarmTrap, "Something Hidden", new List<Trait>(1)
                          {
